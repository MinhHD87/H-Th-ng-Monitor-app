...
import vn.htcjsc.brandname.voice.services.SystemMonitorServiceIF;
...
public class SystemMonitorController extends AbstractBackEnConst {

    private final String REDIRECT_VIEW = "redirect:/admin/system-monitor/list";
    private final String TABLE = "system";
    private final String MODEL_NAME = "systemMonitor";
    @Autowired
    SystemMonitorServiceIF systemMonitorSV;
...
//        }
        model.put(TITLE, LANG.get("systemMonitor.title.list"));
        return "system-monitor";
    }

    @PostMapping("/rest/data")
    @Override
    public ResponseEntity<AngularModel<SystemMonitor>> getData(HttpServletRequest request) {
        AngularModel<SystemMonitor> ngModel = new AngularModel<>();
        ...
        ArrayList<SystemMonitor> listData = systemMonitorSV.view(crPage, maxRow, key, ip, status);
        int totalRow = systemMonitorSV.count(key, ip,content, status);
