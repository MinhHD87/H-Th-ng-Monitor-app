...
<div id="content">
    <div id="content-header">
        <div id="breadcrumb"><a href="<c:url value="/admin/index"/>" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> ${Lang['generic.home']}</a> <a href="<c:url value="/admin/message-monitor/list"/>" class="current">${Lang['message.monitor.subMenu']}</a></div>
    </div>
    <div class="container-fluid">
        <div ng-app="listMessageMonitorApp" ng-controller="listMessageMonitorCtrl" class="row-fluid" ng-init="maxRow = '20';
                        crPage = '1';
                        key = '';
                        phone = '';
                        email = '';
            ...
            <div class="span12">
                <div class="widget-box">
                    <div class="widget-title"> <span class="icon"><i class="icon-align-justify"></i> </span>
                        <h5>${Lang['message.monitor.search.title']}</h5>
...
<!--                                <label class="control-label">SENDTO </label>-->
                                <div class="controls">
                                    ...
                                    <select style="width: 150px;vertical-align: middle" ng-model="status">
                                            ...
                                            <option value="127">${Lang["generic.status.all"]}</option>
                                            <option value="1">${Lang["generic.status.active1"]}</option>
                                            <option value="0">${Lang["generic.status.unactive1"]}</option>
                ...
                <div class="widget-box">
                    <div class="widget-title"> <span class="icon"> <i class="icon-th"></i> </span>
                        <h5>${Lang['message.monitor.detail.title']} <span id="result_model" class="callback_info">${result.message}</span>
                    ...
                    <div class="widget-content nopadding">
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <tr ng-repeat="oneMess in datas">
                                    <td>{{($index + 1)}}</td>
                                    <!--<td>{{oneAcc.accId}}</td>-->
                                    <td>{{oneMess.name}}</td>
                                    <td>{{oneMess.ip}}</td>
                                    <td>{{oneMess.content}}</td>
                                   <td class="img-center">
                                        <img ng-if="oneMess.del" width="24" src="<c:url value="/admin/images/recycle.png"/>" alt="recycle">
                                        <img ng-if="oneMess.status === ${status.ACTIVE}" width="24" src="<c:url value="/admin/images/active.png"/>" alt="active">
                                        <img ng-if="oneMess.status === ${status.UNACTIVE}" width="24" src="<c:url value="/admin/images/cancel.png"/>" alt="inactive">
                                        
                                    </td>
                                    <td>{{oneMess.code}}</td>
                                    <td ng-bind="formatDateTime(oneMess.timesend)"></td>

                                    <td>{{oneMess.type}}</td>
                                    <td>{{oneMess.sendto}}</td>
